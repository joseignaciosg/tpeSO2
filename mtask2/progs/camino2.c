...
static SoftHandler_t old_dos;
static Mutex_t *dos_mutex;
...
mprint(int x, int y, char *format, ...)
{
	...
	EnterMutex(dos_mutex);
	gotoxy(x, y);
	n = vprintf(format, ...);
	clreol();
	LeaveMutex(dos_mutex);
	return n;
}
...
new_dos(InterruptRegs_t regs)
{
	EnterMutex(dos_mutex);
	CallInterrupt(old_dos, &regs);
	LeaveMutex(dos_mutex);
}
...
main(int argc, char **argv)
{
	...
	dos_mutex = CreateMutex("dos");
	old_dos = HookInterrupt(0x21, new_dos);
	kbd = CreateSem("key", 0);
